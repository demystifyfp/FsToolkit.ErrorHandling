...
let ``JobResultCE return! Tests`` =
    testList
        "JobResultCE return! Tests"
        [ testCaseJob "Return Ok Result"
          <| job {
              let innerData = "Foo"
          ...
          testCaseJob "Return Ok AsyncResult"
          <| job {
              let innerData = "Foo"
              let data = Result.Ok innerData
...
let ``JobResultCE bind Tests`` =
    testList
        "JobResultCE bind Tests"
        [ testCaseJob "Bind Ok Result"
          <| job {
              let innerData = "Foo"
              let data = Result.Ok innerData
          ...
          <| job {
              let innerData = "Foo"
              let! actual = jobResult { do! Task.FromResult innerData :> Task }
...
let ``JobResultCE combine/zero/delay/run Tests`` =
    testList
        "JobResultCE combine/zero/delay/run Tests"
...
let ``JobResultCE using Tests`` =
    testList
        "JobResultCE using Tests"
        [ testCaseJob "use normal disposable"
          <| job {
              let data = 42
          ...
          testCaseJob "use! normal wrapped disposable"
          <| job {
              let data = 42
          ...
          testCaseJob "use null disposable"
          <| job {
              let data = 42
...
let ``AsyncResultCE applicative tests`` =
    testList
        ...
        [ testCaseJob "Happy Path JobResult"
          ...
          testCaseJob "Fail Path Result/Choice/AsyncResult"
          <| job {
              let errorMsg = "TryParse failure"
