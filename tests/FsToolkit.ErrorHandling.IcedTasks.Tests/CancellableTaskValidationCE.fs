...
module CancellableTaskValidationCE =
        ...
        testList "CancellableTaskValidationBuilder" [
            ...
            testList "ReturnFrom" [
                ...
                <| fun () ->
                    task {
                        let data = 42
                        let okData: Validation<int, int> = Ok data

                        let ctr = cancellableTaskValidation { return! okData }

                        let! actual = ctr CancellationToken.None
                        Expect.equal actual okData "Should be able to Return! validation"
                    }
    ...
    let ``CancellableTaskValidationCE inference checks`` =
        testList "CancellableTaskValidationCE inference checks" [
    ...
    let cancellableTaskValidationTests =
        testList "CancellableTaskValidation" [
            ...
            functionTests
            ``CancellableTaskValidationCE inference checks``
        ]
