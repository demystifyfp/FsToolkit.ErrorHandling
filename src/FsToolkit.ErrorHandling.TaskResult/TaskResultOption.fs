...
module TaskResultOption =
    let map f tro = TaskResult.map (Option.map f) tro
    ...
    let retn value = task { return Ok(Some value) }

    let apply fTRO xTRO = map2 (fun f x -> f x) fTRO xTRO

    /// Replaces the wrapped value with unit
    let ignore tro = tro |> map ignore
