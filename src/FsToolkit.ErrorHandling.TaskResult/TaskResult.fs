...
module TaskResult = 
  ...
  let inline map f (tr : ^a) =
    Task.map (Result.map f) tr

  let inline  mapError f (tr : ^a) =
    Task.map (Result.mapError f) tr    

  let inline bind (f : _ -> ^b) (tr : ^a) = task {
    match! tr with
    | Ok x -> return! f x
    | Error e -> return Error e
  }

  let inline  foldResult onSuccess onError (tr : ^a) =
  ...
  let inline  map2 f (xTR : ^a) (yTR : ^b) =
    Task.map2 (Result.map2 f) xTR yTR

  let inline  map3 f (xTR : ^a) (yTR : ^b) (zTR : ^c) =
    Task.map3 (Result.map3 f) xTR yTR zTR

  let inline  apply (fTR : ^b) (xTR : ^a)  =
  ...
  let inline  ignoreError (taskResult : ^a) =
  ...
  let inline  teeIf predicate f taskResult =
  ...
  let inline  teeError f taskResult =
  ...
  let inline  teeErrorIf predicate f taskResult =
