...
module TaskResult =
    ...
    let map f tr = Task.map (Result.map f) tr

    let mapError f tr = Task.map (Result.mapError f) tr
    ...
    let retn x = Ok x |> Task.singleton

    let ok = retn

    let returnError x = Error x |> Task.singleton

    let error = returnError

    let map2 f xTR yTR = Task.map2 (Result.map2 f) xTR yTR

    let map3 f xTR yTR zTR = Task.map3 (Result.map3 f) xTR yTR zTR

    let apply fTR xTR = map2 (fun f x -> f x) fTR xTR
    ...
    let tee f taskResult = taskResult |> Task.map (Result.tee f)
    ...
    let ofTask x = x |> Task.map Ok

    /// Lift Result to TaskResult
    let ofResult (x: Result<_, _>) = x |> Task.singleton
