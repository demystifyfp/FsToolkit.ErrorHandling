...
module TaskResult =
    ...
    let inline returnError x = Error x |> Task.singleton
    ...
    let inline tee ([<InlineIfLambda>] f) taskResult = taskResult |> Task.map (Result.tee f)
    ...
    let inline ofTask x = x |> Task.map Ok

    /// Lift Result to TaskResult
    let inline ofResult (x: Result<_, _>) = x |> Task.singleton
