...
let getMethodsByModuleName namesToFilter moduleName (types: Type list) =
    let findWantedModules = List.filter (fun (t: Type) -> t.Name = moduleName)
...
let allModules: list<Module> = [
    ...
    Module.Create "ValueTask" "ValueTask"
    Module.Create "ValueTaskModule" "ValueTask"
    Module.Create "ColdTaskModule" "ColdTask"
    Module.Create "CancellableTaskModule" "CancellableTask"
    Module.Create "CancellableValueTaskModule" "CancellableValueTask"
    Module.Create "CancellableTaskResultModule" "CancellableTaskResult"
]
...
let main argv =
    let types = [
        ...
        yield! Assembly.Load("IcedTasks").GetTypes()
        yield! Assembly.Load("FsToolkit.ErrorHandling.IcedTasks").GetTypes()
    ]
